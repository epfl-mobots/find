...
def plot(exp_files, path, args):
    ...
    if False:
        ...
        def update(val):
            ...
            ax.plot(traj[points[1:], 0],
                    traj[points[1:], 1], linestyle='None', marker='o', color=palette[0])
            ax.plot(traj[points[0], 0],
                    traj[points[0], 1], linestyle='None', marker='X', color=palette[0])

            if num_inds == 2:
                ax.plot(traj[val[0]:val[1], 2],
                        traj[val[0]:val[1], 3], color=palette[1])
                ax.plot(traj[points[1:], 2],
                        traj[points[1:], 3], linestyle='None', marker='o', color=palette[1])
                ax.plot(traj[points[0], 2],
                        traj[points[0], 3], linestyle='None', marker='X', color=palette[1])
    ...
    ax.plot(traj[points[1:], 0],
            traj[points[1:], 1], linestyle='None', marker='o', color=palette[0])
    ax.plot(traj[points[0], 0],
            traj[points[0], 1], linestyle='None', marker='X', color=palette[0], markersize=15)
    ...
    if num_inds == 2:
        ...
        ax.plot(traj[points[1:], 2],
                traj[points[1:], 3], linestyle='None', marker='o', color=palette[1])
        ax.plot(traj[points[0], 2],
                traj[points[0], 3], linestyle='None', marker='X', color=palette[1], markersize=15)
